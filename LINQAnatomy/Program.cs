...
namespace LINQAnatomy
{
	internal class Program
	{
		...
		private static void SomeTest()
		{
			...
			var values = Enumerable.Range(0, max);

			//for (var i = 0; i < max; i++)
			//{
			//	values.Add(random.Next(max));
			//}
			// get even numbers orderd and take 10
			var stop = new Stopwatch();
			stop.Start();
			var res1 = values.OrderBy(v => v).Distinct().Where(v => v % 2 == 0).Take(10).ToList();
			stop.Stop();
			var res1Time = stop.Elapsed;
			Console.WriteLine($"res1 {res1Time}");
			stop.Reset();
			stop.Start();
			var res2 = values.Where(v => v % 2 == 0).Distinct().OrderBy(v => v).Take(10).ToList();
			stop.Stop();
			var res2Time = stop.Elapsed;
			Console.WriteLine($"res2 {res2Time}");

			Console.WriteLine($"res2 faster than {(res1Time.TotalMilliseconds / res2Time.TotalMilliseconds) * 100}");
